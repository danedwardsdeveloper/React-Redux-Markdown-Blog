...
function FilterTermPage() {
	const {
		...
		visibleArticles,
		totalPages,
	} = useSelector((state) => state.articlesSlice);
	...
	useEffect(() => {
		let title =
			totalPages === 1
				? `${filterTerm} | Array of Sunshine | Front-End WebDev Blog: JavaScript, React, Coding Book Reviews`
				: `${filterTerm}, Page ${currentPage} | Array of Sunshine | Front-End WebDev Blog: JavaScript, React, Coding Book Reviews`;

		document.title = title;
	}, [currentPage, filterTerm, totalPages]);
